...
export const archiveTestCase = async (req: Request, res: Response): Promise<void> => {
  ...
  const testCase = await testCaseService.archive(testCaseId);
  res.status(200).json(ok(testCase, { message: Messages.TEST.ARCHIVED }));
};