...
export const analyticsService = {
  async qualityScore(params: QualityScoreParams): Promise<QualityScore> {
    return cacheService.remember(`analytics:quality:${params.projectId}`, async () => {
      ...
      const testCoverage = resultStats._count > 0 ? Math.min(1, (passedRuns / Math.max(1, totalRuns)) * (runs.length / 30)) : 0;
  ...
  async releaseRisk(params: QualityScoreParams): Promise<{ riskScore: number; riskLevel: 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL'; details: Record<string, number> }> {
    const quality = await analyticsService.qualityScore(params);
    const [openCriticalBugs, recentRuns] = await Promise.all([
      prisma.bug.count({
        where: { projectId: params.projectId, status: { not: 'CLOSED' }, severity: { in: ['CRITICAL', 'HIGH'] } },
      }),
      prisma.testRun.findMany({
        where: { projectId: params.projectId },
        orderBy: { createdAt: 'desc' },
    ...
    const failureRate = recentRuns.length > 0 ? recentRuns.filter((run) => run.status === 'FAILED').length / recentRuns.length : 0;
    const riskScore = Math.round(
      (100 - quality.score) * 0.5 + (100 - quality.components.passRate) * 0.2 + openCriticalBugs * 5 + failureRate * 100 * 0.3
    );
  ...
  async getDashboardAnalytics(organizationId: string, projectId?: string): Promise<unknown> {
    const [projects, recentRuns, openBugs, totalCases] = await Promise.all([
      ...
      prisma.bug.count({
        where: { ...(projectId ? { projectId } : { project: { organizationId } }), status: { not: 'CLOSED' } },
      }),
...
const estimateFlakiness = (runs: Array<{ passedTests: number; failedTests: number; totalTests: number }>): number => {
  const runsWithResults = runs.filter((run) => run.totalTests > 0);
  if (runsWithResults.length === 0) return 0;
  const inconsistent = runsWithResults.filter(
    (run) => run.passedTests > 0 && run.failedTests > 0
  ).length;
