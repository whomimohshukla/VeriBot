...
export const projectRepository = {
  ...
  findActiveById: (id: string) => prisma.project.findFirst({ where: { id, archivedAt: null } }),

  create: (data: Prisma.ProjectUncheckedCreateInput) => prisma.project.create({ data }),

  update: (id: string, data: Prisma.ProjectUpdateInput) => prisma.project.update({ where: { id }, data }),

  hardDelete: (id: string) => prisma.project.delete({ where: { id } }),

  archive: (id: string) => prisma.project.update({ where: { id }, data: { archivedAt: new Date() } }),
