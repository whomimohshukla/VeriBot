...
import { createApplication, listApplicationsForProject } from "../services/application.service";
...
export const createApplicationController = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(400).json({ message: error instanceof Error ? error.message : "Application creation failed" });
  }
};
